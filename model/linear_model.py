...
class NumpyLinearModel():
    ...
    def loss_v(self, v_pred:np.ndarray, v_trgt:np.ndarray) -> float:
        ...
        ########################
        B = v_pred.shape[0]
        return np.sum((v_trgt - v_pred) ** 2) / B
    ...
    def grad_v(self, X:np.ndarray, v_trgt:np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # 计算tanh的导数
        tanh_derivative = 1 - v_pred ** 2
        
        # 计算误差项
        error = v_pred - v_trgt
        
        # 计算梯度
        grad_w_v = (2/B) * (X.T @ (error * tanh_derivative)).flatten()
        grad_b_v = (2/B) * np.sum(error * tanh_derivative)
